...
with col2:
    with st.spinner('Calculating statistics...'):
        ...
        def color_change(val):
            if not val:  # Empty string for Volume
                return ''
            color = 'green' if float(val.strip('%').replace('+', '').replace(',', '')) >= 0 else 'red'
            return f'color: {color}'
        
        styled_stats = stats.style.format({
            'Value': lambda x: x,  # Keep the existing formatting
            'Change': lambda x: x   # Keep the existing formatting
